...
def convert_node_names(nodes: List[Dict], links: List[Dict]) -> Tuple[Dict[str, str], Dict[str, Dict]]:
    """
    ...
    for layer_feature, node_list in consolidated_features.items():
        ...
        node_info[layer_feature] = {
            ...
            "max_influence": max(influence_by_position.values()) if influence_by_position else 0.0,
...
def build_adjacency_list(name_mapping: Dict[str, str], links: List[Dict]) -> Dict[str, List[str]]:
    """
...
class ExplanationPipeline:
    ...
    def __init__(self, topo_order: List[str], adjacency_list: Dict[str, List[str]], node_info: Dict[str, Dict]):
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "progress": len(self.explained) / len(self.topo_order) if self.topo_order else 0.0
        }
    
    def create_latent_dict_for_nodes(self, ready_nodes: List[str]) -> Dict[str, torch.Tensor]:
...
def add_parent_connections_to_all_nodes(
    ...
    parent_relationships = defaultdict(lambda: defaultdict(float))  # {child_cantor: {parent_cantor: max_weight}}
    ...
    for link in graph["links"]:
        ...
        if source_cantor is not None and target_cantor is not None:
            ...
            parent_relationships[target_cantor][source_cantor] = max(current_weight, weight)
...
async def generate_explanations_from_cache(
    ...
    name_mapping, node_info = convert_node_names(graph_data["nodes"], graph_data["links"])
    ...
    print(f"Prepared latent dict with {sum(len(v) for v in latent_dict.values())} features across {len(latent_dict)} modules")
    ...
    def explanation_postprocess(result):
        ...
        layer = result.record.latent.module_name.split('.')[-2]  # Extract layer number from "layers.X.mlp"
        ...
        print(f"   Explained feature {node_name}")
        return result
    explainer_pipe = Pipe(
    ...
    async def explain_with_pipeline():
        ...
        while not explanation_pipeline.is_complete():
            ...
            ready_nodes = explanation_pipeline.get_ready_nodes(batch_size=len(explanation_pipeline.queue))
            
            if not ready_nodes:
                print("No ready nodes, but pipeline not complete - checking for issues...")
                status = explanation_pipeline.get_status()
                print(f"Status: {status}")
                break
            
            print(f"Iteration {iteration}: Processing {len(ready_nodes)} features")
            print(f"Features: {ready_nodes}")
            
            # Create latent_dict for this iteration's nodes
            iteration_latent_dict = explanation_pipeline.create_latent_dict_for_nodes(ready_nodes)
            print(f"Created latent dict with {sum(len(v) for v in iteration_latent_dict.values())} features across {len(iteration_latent_dict)} modules")
            ...
            # Show progress
            status = explanation_pipeline.get_status()
            print(f"Progress: {status['explained']}/{status['total_nodes']} "
                  f"({status['progress']:.1%}) - {status['queued']} queued")
        
        return explanation_pipeline.explanations
    
    # Run the pipeline-based explanation process
    all_explanations = await explain_with_pipeline()
    
    print(f"Explanation generation complete! Results saved to {output_dir}")
    print(f"   Generated {len(all_explanations)} explanations")
    print(f"   Processing followed topological order to ensure dependencies")
...
async def main():
    ...
    output_dir = "results/graph_enhanced_llama70b"
