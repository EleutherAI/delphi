...
try:
    ...
    # Using more_itertools for compatibility
import gc
...
parser.add_arguments(
    SamplerConfig,
    dest="sampler_cfg",
    default=SamplerConfig(n_examples_train=40, n_quantiles=10, train_type="quantiles"),
)
...
# Utility function to set activation buffer on a record (not used in main loop)
...
# Try to construct lm_head for different HuggingFace architectures
if hasattr(cache_lm, "model") and hasattr(cache_lm.model, "norm") and hasattr(cache_lm.model, "lm_head"):
    # llama models
    lm_head = torch.nn.Sequential(cache_lm.model.norm, cache_lm.model.lm_head)
elif hasattr(cache_lm, "gpt_neox") and hasattr(cache_lm, "embed_out"):
    # pythia models
    lm_head = torch.nn.Sequential(cache_lm.gpt_neox.final_layer_norm, cache_lm.embed_out)
else:
